...
if TYPE_CHECKING:
    ...
    from ..models.pager_duty_api_key_store_id import PagerDutyAPIKeyStoreID
    from ..models.pager_duty_explicit_key import PagerDutyExplicitKey


T = TypeVar("T", bound="PagerDutyPluginCreate")


@_attrs_define
class PagerDutyPluginCreate:
    """
    Attributes:
        plugin_name (Any): The PagerDuty plug-in. Default: 'pagerduty'. Example: pagerduty.
        integration_key (PagerDutyAPIKeyStoreID | PagerDutyExplicitKey):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        integration_key: dict[str, Any]
        if isinstance(self.integration_key, PagerDutyExplicitKey):
            integration_key = self.integration_key.to_dict()
        else:
            integration_key = self.integration_key.to_dict()
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        def _parse_integration_key(
            data: object,
        ) -> PagerDutyAPIKeyStoreID | PagerDutyExplicitKey:
            try:
                if not isinstance(data, dict):
                    raise TypeError()
                componentsschemas_pager_duty_store_or_integration_key_selector_type_0 = PagerDutyExplicitKey.from_dict(
                    data
                )

                return componentsschemas_pager_duty_store_or_integration_key_selector_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            if not isinstance(data, dict):
                raise TypeError()
            componentsschemas_pager_duty_store_or_integration_key_selector_type_1 = (
                PagerDutyAPIKeyStoreID.from_dict(data)
            )

            return componentsschemas_pager_duty_store_or_integration_key_selector_type_1

        integration_key = _parse_integration_key(d.pop("integration_key"))
        ...
        pager_duty_plugin_create.additional_properties = d
        return pager_duty_plugin_create
